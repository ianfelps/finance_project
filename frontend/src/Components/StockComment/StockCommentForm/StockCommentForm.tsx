...
const StockCommentForm = ({ symbol, handleComment }: Props) => {
    const { register, handleSubmit, formState: { errors }, reset } = useForm<CommentFormInputs>({ resolver: yupResolver(validation)});
  ...
  return (
    <form className="mt-4 ml-4" onSubmit={handleSubmit(onSubmit)}>
      <input
        ...
        {...register("title")}
      />
      {errors.title ? <p className="text-red-500 my-2">{errors.title.message}</p> : ""}
      <div className="py-2 px-4 mb-4 bg-white rounded-lg rounded-t-lg border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
        ...
        {errors.content ? <p className="text-red-500 my-2">{errors.content.message}</p> : ""}
      <button
        type="submit"
